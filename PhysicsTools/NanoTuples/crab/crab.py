...
def main():
    ...
    # disable outputdir options here
    # disable site options here
    # disable tag options here
    # disable pset options here
    # disable inputfile options here
    # disable units-per-job options here

    if os.getenv('USER') == "qiguo":
        args.outputdir = "/store/user/qiguo/B2G/1lep/SF/Customized_NanoAOD/MC_2018/"
        args.site = "T2_CN_Beijing"
        args.tag = "NanoTuples-v2"
        args.pset = "mc2018_NANO.py"
        args.inputfile = "mc_test.txt" # 6.16 , just to Test VVV samples
        Submit_Scripts = "/eos/user/q/qiguo/B2G/1lep/Private_NanoAOD/UL/V1/CMSSW_10_6_19_patch2/src/submit_test.sh" # 6.16 , just to Test VVV samples
        Submit_Scripts_f = open(Submit_Scripts,"w")
        args.units_per_job = 1
        



    # don't know if we need to use "debug" section
    ...
    with open(args.inputfile) as inputfile:
        for l in inputfile:
            ...
            if args.dryrun:
                print('-' * 50)
                print(cfg)
                Submit_Scripts_f.write('crab submit -c {cfgpath}'.format(cfgpath=os.getcwd()+'/'+cfgpath))
                continue
